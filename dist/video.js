...
export default class Carousel {
  ...
  #listen() {
    ...
    for (let item of this.elements.snaps) this.carousel_observer.observe(item);
  ...
  #initializeState() {
    ...
    this.elements.snaps.forEach((snapChild, index) => {
      ...
      this.elements.pagination.appendChild(
        this.#createMarker(snapChild, index)
      );
  ...
  #handleScrollStart() {
    if (this.elements.root.hasAttribute('carousel-start')) {
      ...
      this.elements.snaps.forEach(
        (snap) => (snap.style.scrollSnapAlign = 'unset')
      );
      ...
      startElement.addEventListener(
        ...
        (e) => {
          ...
          this.elements.snaps.forEach(
            (snap) => (snap.style.scrollSnapAlign = null)
          );
        },
        { once: true }
      );
  ...
  #createMarker(item, index) {
    ...
    else return this.#createMarkerDot({ index, type: markerType, item });
  }
